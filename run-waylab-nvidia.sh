...
Launcher for Waylab on NVIDIA PRIME / GBM with categorized, timestamped multi-level logging.
...
Examples:
  ./run-waylab-nvidia.sh
  ./run-waylab-nvidia.sh --tee
  ./run-waylab-nvidia.sh --damage -w 1
  ./run-waylab-nvidia.sh --gdb
EOF
    exit 0
}
...
echo "=== Launching Waylab on NVIDIA ==="
echo "Config dir     : $CONFIG_DIR"
echo "DRM devices    : $WLR_DRM_DEVICES"
echo "GBM Backend    : $GBM_BACKEND"
